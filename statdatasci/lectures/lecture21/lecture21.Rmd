...
- Recall: z-scores corresponding to particular probabilities are often written as $z_p$, where $p$ denotes the probability, e.g., $z_{.025} \approx -1.96$

- Set up: $X \sim N(\mu, \sigma^2)$. We are interested in a confidence interval for the unknown population parameter $\mu$. We use the sample mean, $\overline{X}$, constructed from a sample of size $n$, as an estimator for the population mean. Assume $n$ is large.
...
- $z_{\frac{\alpha}{2}}$ is the z-score corresponding to a probability of $\frac{\alpha}{2}$ in the left tail. We saw earlier that for $\alpha = .05$, $z_{\frac{\alpha}{2}}$ is -1.96, i.e., $P(Z \leq -1.96) \approx .025$.
