...
- Given a hypothesis about $\mu$ that we wish to test, $\mu = \mu_0$, we simply check if the value $\mu_0$ falls within the constructed confidence interval

- If it falls within CI, do not reject. If it doesn't, reject. 
...
- To show this is a valid hypothesis test (controls type I error), we need to show that P(reject $H_0$ when $H_0$ true) = $.05$ using our proposed rejection rule 
...
**Decision rule** for an $\alpha$-level test for $H_0: \mu = \mu_0$
...
- Construct a $100(1-\alpha)$% confidence interval for $\mu$

- Given a hypothesis $\mu = \mu_0$, check if $\mu_0$ falls within the constructed confidence interval. If it does, do not reject our hypothesis. If it doesn't, reject. 
...
- Typically, hypothesis tests are **two-sided**
  
  - Looking for any difference from a hypothesized value, whether smaller or larger. 

- Sometimes, it is appropriate to conduct a **one-sided** test

  - For example, in the lead example, we might only be interested in whether the lead level is greater than the EPA action level
...
How do we get the p-value in R?

--

`1 - pnorm(z)`, where `z` is the value of the test statistic
...
`pnorm(z)`, where `z` is the value of the test statistic
...
Now, we are only interested in short trees. We would like to do a test with the alternative that $\mu < 210$. We collect data on the heights of 300 randomly sampled redwood trees. Assume the samples are indepedent. We get a sample mean of 220 and sample standard deviation of 20. Conduct a hypothesis test at a 1% significance level. 
...
**Critical value**: $z_{.025} \approx 1.96$
