...
### Vectors 
Use `sample()`, sampling *with* replacement, to create a vector of length 20. (Use the help command `?sample` to learn about the function.) Print it and check if there are missing values. Find the sum and mean of the vector. Assign the 5th value of the vector the value 0. Create a new vector which only has elements of the first vector that are larger than 5.
...
### Matrices
Create a $3 \times 3$ matrix containing the elements 1 to 9 (filled *row-wise*). Multiply the matrix by 3, element-wise. Post-multiply the matrix by a $3 \times 1$ vector (5, 5, 1). Pre-multiply the transpose of the matrix by a $3 \times 1$ vector (5, 5, 1).
